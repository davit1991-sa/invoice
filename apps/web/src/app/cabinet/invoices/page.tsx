...
export default function InvoicesPage() {
  ...
  const total = useMemo(() => rows.reduce((s, r) => s + Number(r.amountGross || 0), 0), [rows]);
  ...
  async function load() {
    ...
    try {
      ...
      const data = await getWithAuth<InvoiceRow[]>(`/invoices?${params.toString()}`);
      setRows(data);
    } catch (e: any) {
  ...
  async function markPaid(id: string) {
    setErr(null);
    try {
      await jsonWithAuth(`/invoices/${id}`, "PUT", { status: "PAID" });
      await load();
    } catch (e: any) {
  ...
  async function remove(id: string) {
    setErr(null);
    try {
      await jsonWithAuth(`/invoices/${id}`, "DELETE", {});
      await load();
    } catch (e: any) {
...
async function downloadPdf(id: string, invoiceNumber: string) {
  setErr(null);
  try {
    const blob = await downloadBlobWithAuth(`/invoices/${id}/pdf`);
    triggerBrowserDownload(blob, `invoice-${invoiceNumber}.pdf`);
  } catch (e: any) {
...
return (
    ...
    <div className="grid gap-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <h1 className="text-xl font-semibold tracking-tight text-slate-900">My Invoices</h1>
          <div className="mt-1 text-sm text-slate-600">
            Total invoices: {rows.length} · Sum: {total.toFixed(2)} GEL
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Link href="/cabinet/invoices/new"><Button>Create Invoice</Button></Link>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-2 md:items-center">
        <input
          ...
          placeholder="Search invoice number / client / purpose"
          value={q}
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.id} className="border-t border-slate-100">
                  ...
                  <td className="px-4 py-3 text-right">{Number(r.amountGross).toFixed(2)}</td>
  ...
  <Button size="sm" variant="outline" onClick={() => sendEmail(r.id, r.client?.email)}>Send Email</Button>
                      ...
                      <Button size="sm" variant="outline" onClick={() => downloadPdf(r.id, r.invoiceNumber)}>Download PDF</Button>
